...
import {ConfirmationService, LazyLoadEvent, MenuItem, SelectItem, SortEvent, MessageService} from 'primeng/api';
import {Course} from '../../../shared/interfaces/course';
import {BreadcrumbService} from '../../../shared/service/breadcrumb.service';
import {SpecialApprove} from '../../../shared/interfaces/special-approve';
import {Router} from '@angular/router';
import {of, UnsubscriptionError, from} from 'rxjs';
...
export class CoursesListComponent implements OnInit {
  ...
  public assignCourse() {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.courseService.assignCourse(dataCourse).subscribe((res) => {
          ...
          if (res['result'] === 'Success') {
            ...
            this.onCancle('as');
            this.messageService.add({severity: 'success', summary: 'ข้อความจากระบบ', detail: 'ดำเนินการลงทะเบียนคอร์ส ' + name + ' สำเร็จ'});
            this.getData();
            this.getStudyingData();
            this.getTotalRecord();
            this.getStudyTotalRecord();
      ...
      reject: () => {
        this.onCancle('as');
        this.messageService.add({severity: 'info', summary: 'ข้อความจากระบบ', detail: 'ยกเลิกกการลงทะเบียน'});
        this.getData();
        this.getStudyingData();
        this.getTotalRecord();
        this.getStudyTotalRecord();
  ...
  private getData() {
    this.values = [];
    this.courseService.getCoursesUser('0')
    .subscribe(res => {
  ...
  private getStudyingData() {
    this.values = [];
    this.courseService.getCoursesUser('2')
    .subscribe(res => {
      if (res['status'] === 'Success' ) {
  ...
  public cancelApprovalCourse(id) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.courseService.cancelApprovalCourse(id).subscribe((res) => {
          ...
          if (res['result'] === 'Success') {
            ...
            this.messageService.add({severity: 'success', summary: 'ข้อความจากระบบ', detail: 'ดำเนินการยกเลิกการขออนุมัติพิเศษสำเร็จ'});
            this.getData();
            this.getStudyingData();
            this.getTotalRecord();
            this.getStudyTotalRecord();
      ...
      reject: () => {
        this.messageService.add({severity: 'info', summary: 'ข้อความจากระบบ', detail: 'ปฏิเสธการยกเลิกการขออนุมัติพิเศษ'});
      }
    });
  }
