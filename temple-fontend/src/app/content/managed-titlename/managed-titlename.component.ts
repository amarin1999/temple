...
export class ManagedTitlenameComponent implements OnInit,AfterViewInit {
  ...
  titlesForm = new FormGroup({
    titleName: new FormControl(null, [Validators.required, Validators.maxLength(45)]),
    titleNameDisplay: new FormControl(null, [Validators.required, Validators.maxLength(45)])
  });
  public formError = {
    titleName: '',
    titleNameDisplay: ''
    ...
    if (!this.titlesForm.valid) {
      ...
      this.titleNamesService.createTitleName(titleName)
        .subscribe(res => {
          ...
          (e) => {
            // console.log(e);
            this.messageService.add({ severity: 'error', summary: 'ข้อความจากระบบ', detail: 'ดำเนินการบันทึกไม่สำเร็จ (คำนำหน้าชื่อหรือคำย่ออาจมีอยู่แล้ว)', life: 5000});
          }
        );
      this.clear();
    }
  ...
  delete(id) {
    ...
    this.confirmationService.confirm({
      message: '(ถ้า) คำนำหน้าถูกใช้งานไม่สามารถลบคำนำหน้าได้',
