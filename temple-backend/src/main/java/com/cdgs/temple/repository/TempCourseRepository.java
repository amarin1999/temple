...
public interface TempCourseRepository extends CrudRepository<TempCourseEntity, Long> {
	...
	@Query(value = "SELECT t1.*,  (CASE WHEN t1.mhc_status='1' THEN 'สำเร็จการศึกษา'  "
			+ "WHEN t1.mhc_status='2' AND t1.course_st_date > NOW() THEN 'รอศึกษา' WHEN t1.course_st_date <= NOW() THEN 'กำลังศึกษา' ELSE (CASE WHEN t1.sa_status='2' THEN 'รอการอนุมัติ' "
