...
public interface TempSpecialApproveRepository extends CrudRepository<TempSpecialApproveEntity, Long> {
	...
	@Query(value = "SELECT sa.special_approve_id, " + "sa.member_id, sa.spa_detail, "
			+ "CONCAT(t.title_name,m.member_fname,' ',m.member_lname) AS display_name, " + "c.course_detail, "
			+ "c.course_st_date, c.course_end_date, " + "ts.tran_name AS transportation " + "FROM special_approve sa "
