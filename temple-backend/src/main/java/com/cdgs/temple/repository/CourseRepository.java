...
public interface CourseRepository extends CrudRepository<CourseEntity, Long> {
	...
	@Query(value ="select count(c.course_id) from courses c " + 
			"INNER JOIN members_has_courses mhc  ON c.course_id = mhc.course_id " + 
			"where 1=1 AND "+ 
			"(CASE WHEN :enable = '*' THEN mhc.mhc_status = :status " + 
			"ELSE mhc.mhc_status = :status AND c.course_enable = :enable " + 
			"END) AND "+
			"mhc.member_id = :memberId " 
			, nativeQuery = true)
	Integer countCourseByEnableIsTrueAndStatus(@Param("memberId") Long memberId, @Param("status") String status, @Param("enable") String enable);	
	...
	@Query(value = "SELECT COUNT(c.course_id) FROM courses c "
			...
			+ "AND c.course_enable = '1'", nativeQuery = true)
	Integer countCoursesTeacherAll(@Param("memberId") Long memberId);
	
	@Query(value = "SELECT COUNT(mhc.course_id) FROM members_has_courses mhc "
	...
	@Query(value = "SELECT c.* FROM courses c "
			+ "INNER JOIN courses_teacher ct "
			+ "ON c.course_id=ct.course_id "
			+ "INNER JOIN special_approve sa "
			+ "ON c.course_id=sa.course_id "
			+ "WHERE 1=1 AND ct.member_id = :memberId "
			+ "AND sa.spa_status='2' "
	...
	@Query(value = "SELECT COUNT(t1.course_id) FROM (" +
	...
	@Query(value = "SELECT * FROM courses c  "
			...
			+ "GROUP BY c.course_id "
			+ "ORDER BY c.course_st_date,c.course_id " 
			, nativeQuery = true)
	...
	List<CourseEntity> fetchCoursesTeacherApprovalOutTime(@Param("memberId") Long memberId, @Param("offset") int offset, @Param("limit") int limit, @Param("query") String query);
